...
def t_pricer_end_to_end():
    ...
    # Near an up-barrier the UO-call delta is small; assert it is bounded/small
    # rather than a fixed sign (the smile overlay can tilt it either way).
    check("UO call delta small (|Δ|<0.15)", abs(r_uo.greeks["delta"]) < 0.15,
          f"Δ={r_uo.greeks['delta']:+.4f}")
...
def t_pipeline():
    print("\n[data pipeline]")
    # Robust to either layout: project as a subdirectory (run from parent)
    # or as the working directory itself (run from inside project/).
    cand_paths = [
        os.path.join(_ROOT, "project", "data", "sample_market_data.csv"),
        os.path.join(_ROOT, "data", "sample_market_data.csv"),
        os.path.join(os.path.dirname(__file__), "data", "sample_market_data.csv"),
    ]
...
def main():
    ...
    for fn in (t_blackscholes, t_delta, t_curves, t_barrier_parity, t_touch_parity,
               t_pde_vs_analytic, t_mc_vs_analytic,
