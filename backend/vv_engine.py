...
def _pillar_matrix_and_costs(slice_):
    """Return (labels, M, costs, Ks, phis) for the three replication pillars.

    M[i,j] = i-th Greek (vega/vanna/volga) of pillar j at the ATM vol.
    costs[lab] = market smile cost of pillar lab = BS(smile_vol) - BS(atm_vol).
    """
    S, T, rd, rf, atm, Ks, sm, phis = _pillar_data(slice_)
    labels = ["ATM", "25C", "25P"]
    M = np.zeros((3, 3))
    ...
    for lab in labels:
        ...
        costs[lab] = (bs.price(S, Kp, T, rd, rf, sm[lab], ph)
                      - bs.price(S, Kp, T, rd, rf, atm, ph))
    return labels, M, costs, Ks, phis


def _survival_p(slice_, H, attenuation, atten_value):
    """Resolve the survival (no-touch) probability used to attenuate the overlay."""
    S, T, rd, rf = slice_.S, slice_.T, slice_.rd, slice_.rf
    atm = slice_.quotes.atm
    if attenuation == "none":
        return 1.0
    if attenuation == "value":
        if atten_value is None:
            raise ValueError("attenuation='value' requires atten_value in [0, 1]")
        p = float(atten_value)
        if not math.isfinite(p) or p < 0.0 or p > 1.0:
            raise ValueError(f"atten_value must be in [0, 1], got {p}")
        return p
    if attenuation == "survival":
        nt = tch.no_touch(S, H, T, rd, rf, atm, 1.0)
        p = float(nt.no_touch_prob)
        if not math.isfinite(p):
            p = 0.0
        return max(0.0, min(1.0, p))
    raise ValueError(f"unknown attenuation mode {attenuation!r}; expected "
                     f"'survival' | 'none' | 'value'")


def barrier_vv_replication(slice_: VolSurfaceSlice, K, H, phi, kind,
                           bs_barrier_price,
                           barrier_vega, barrier_vanna, barrier_volga,
                           attenuation="survival", atten_value=None) -> VVResult:
    """VV overlay for a barrier built on the BARRIER's OWN sensitivities.

    This is the standard market / Bloomberg-style vanna-volga-for-exotics
    construction (Castagna-Mercurio 2007; Bossens, Rayee, Skantzos, Deelstra
    2010; Wystup). Instead of taking the full vanilla smile correction and
    scaling it (which uses the *vanilla's* vega/vanna/volga), we solve the same
    3x3 replication system with the *exotic's* vega/vanna/volga as the target,
    so the overlay charges the market smile cost of hedging the barrier's actual
    smile risk. For knock-outs the correction is attenuated by the survival
    (no-touch) probability p, since a knocked-out option never realises the smile.

    barrier_vega/vanna/volga : BS Greeks of the barrier at the ATM vol, in the
        same per-unit-vol convention as blackscholes.vega/vanna/volga.
    """
    labels, M, costs, Ks, phis = _pillar_matrix_and_costs(slice_)
    tgt = np.array([float(barrier_vega), float(barrier_vanna), float(barrier_volga)])
    try:
        w = np.linalg.solve(M, tgt)
    except np.linalg.LinAlgError:
        w = np.linalg.lstsq(M, tgt, rcond=None)[0]
    ...
    full_adj = sum(weights[l] * costs[l] for l in labels)
    p = _survival_p(slice_, H, attenuation, atten_value)
    correction = p * full_adj
    vv_price = bs_barrier_price + correction
    if not math.isfinite(vv_price):
        vv_price = 0.0

    vanna_cost = weights["25C"] * costs["25C"] - weights["25P"] * costs["25P"]
    volga_cost = full_adj - vanna_cost
    return VVResult(bs_barrier_price, vv_price, correction, weights,
                    p * vanna_cost, p * volga_cost,
...
def barrier_vv(slice_: VolSurfaceSlice, K, H, phi, kind,
    ...
    attenuation      : 'survival' -> weight correction by no-touch prob p
                       ...
                       'value'    -> use the supplied atten_value (must be in [0,1])
    """
    S, T, rd, rf = slice_.S, slice_.T, slice_.rd, slice_.rf
    atm = slice_.quotes.atm
    ...
    if attenuation == "none":
        p = 1.0
    elif attenuation == "value":
        if atten_value is None:
            raise ValueError("attenuation='value' requires atten_value in [0, 1]")
        p = float(atten_value)
        if not math.isfinite(p) or p < 0.0 or p > 1.0:
            raise ValueError(f"atten_value must be in [0, 1], got {p}")
    elif attenuation == "survival":
        nt = tch.no_touch(S, H, T, rd, rf, atm, 1.0)
        p = float(nt.no_touch_prob)         # survival probability
        if not math.isfinite(p):
            p = 0.0
        p = max(0.0, min(1.0, p))
    else:
    ...
    vv_price = bs_barrier_price + correction
    # Numerical guard: a barrier price cannot be negative (zero rebate case).
    if not math.isfinite(vv_price):
        vv_price = 0.0
