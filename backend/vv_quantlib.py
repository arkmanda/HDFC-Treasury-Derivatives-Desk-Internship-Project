...
vv_quantlib.py
==============
A self-contained, pure-Python reimplementation of QuantLib's
``VannaVolgaBarrierEngine`` (Yue Tian, 2013) and the underlying
``AnalyticBarrierEngine`` / ``BlackDeltaCalculator`` it relies on.

The goal is bit-for-bit agreement with QuantLib's vanna-volga barrier prices
so the desk can reproduce the reference engine's numbers without a QuantLib
dependency. Every formula below is transcribed from the QuantLib sources:

  * analyticbarrierengine.cpp     -> the A..F Reiner-Rubinstein terms
  * blackdeltacalculator.cpp      -> strike-from-delta and the ATM strike
  * vannavolgabarrierengine.cpp   -> the overlay: solve a 3x3 for the barrier's
                                     own vega/vanna/volga, weight the 25C/25P
                                     smile costs by the survival probability
  * vannavolgainterpolation.hpp   -> the second-order VV smile used to vol the
                                     vanilla leg (and hence the in-barrier)

Conventions (matching the QuantLib FX setup
``BlackScholesMertonProcess(spot, foreignTS, domesticTS, vol)``):
...
# --------------------------------------------------------------------------- #
# Black (Garman-Kohlhagen in forward form, exactly QuantLib's blackFormula)    #
# --------------------------------------------------------------------------- #
...
def strike_from_delta(phi: int, delta: float, F: float, stddev: float,
    ...
    """Exact transcription of QuantLib BlackDeltaCalculator::strikeFromDelta."""
    if dtype == "spot":
    ...
    # premium-adjusted: solve numerically (QuantLib uses Brent with bounds)
...
def atm_strike(F: float, stddev: float, dtype: str) -> float:
    """Delta-neutral-straddle ATM strike (QuantLib AtmDeltaNeutral)."""
...
# --------------------------------------------------------------------------- #
class _BarrierBS:
    """Exact port of QuantLib's AnalyticBarrierEngine (continuous monitoring)."""
...
def price_vv_barrier(S, K, H, T, rd, rf, atm, rr25, bf25,
    ...
    """QuantLib-faithful vanna-volga barrier price.
    ...
    # already-triggered shortcuts (QuantLib handles these explicitly)
    if up and S >= H:
        if barrier_type == "uo":
    ...
    # barrier BS greeks by finite difference (QuantLib's exact shifts)
    dsig = 0.0001
    dS = 0.0001 * S
